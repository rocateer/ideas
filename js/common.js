...
document.addEventListener("DOMContentLoaded", function() {
  const menuWraps = document.querySelectorAll(".dot_menu_wrap");

  menuWraps.forEach(function(menuWrap) {
    const button = menuWrap.querySelector(".dot_button");
    const menu = menuWrap.querySelector(".dot_layer");

    if (!button || !menu) return;

    button.addEventListener("click", function(e) {
      e.stopPropagation();
      document.querySelectorAll(".dot_menu_wrap.open").forEach(function(other) {
  ...
  document.addEventListener("click", function() {
    document.querySelectorAll(".dot_menu_wrap.open").forEach(function(w) {
...
$(function(){
 ...
 // slide 탭
$(".slide_tab .item_tab").click(function() {
  $(".slide_tab .item_tab").removeClass("active");
  $(this).addClass("active");
});
...
$(".master_open").click(function(){
  $(this).siblings('div').removeClass("active");
  $(this).addClass("active");
  $(".profile_layer").stop().slideToggle(0);
  return false;
});


//side 메뉴
document.querySelectorAll('.menu_wrapper').forEach(function(menuWrapper) {
  var sideMenu = menuWrapper.querySelector('.menu_item');
  var subMenu = menuWrapper.querySelector('.my_submenu');
  var menuArrow = menuWrapper.querySelector('.menu_arrow');
  ...
  document.querySelectorAll(".custom_select").forEach(select => {
    ...
    optionsList.forEach(option => {
      option.addEventListener("click", function () {
        ...
        options.style.display = "none"; // 옵션 닫기
      });
    });
...
$(".btn_q").click(function(){
  $(this).siblings(".layer_info").stop().slideToggle(300);
  return false;
});
...
$(".toggle_btn").click(function(){
  $(".toggle_layer").stop().slideToggle(0);
  return false;
});
  ...
  $(document).ready(function(){
    $("#search_input").on("propertychange change keyup paste input", function(){
        if ($(this).val().length === 0) {
            $('.input_delete').css('display','none');
        } else {
            $('.input_delete').css('display','inline-block');
        }
    });
...
$(".search_toggle").click(function(){
  $(".new_search_wrap").stop().slideToggle(300);
  return false;
});
...
function sideNavToggle(){
  $("header, .nav_dim").toggleClass("open");
...
function filterToggle(){
  $(".fiter_wrap").toggleClass("open");
  if($(".fiter_wrap").hasClass("open")){
    $.lockBody();
  } else {
    $.unlockBody();
  }
...
$(document).on("click",".accordion .trigger",function(){
  ...
  var dropdown_arrow = $(this).find(".toggle_arrow");
